...
class inpostshipping
{
	...
	private function output_row1( $shipping )
	{
		...
		$sel2_drop = '';
		if ( empty( $shipping['label_type'] ) )
		{
			$sel2_drop = '';
		}
		else
		{
			$sel2_drop = $shipping['label_type'];
		}
		?>
		<tr>
			<td<?php echo $class; ?>>
				<div class="cell-wrapper">
					<input type="text" name="wpsc_shipping_inpostshipping_shipping[weight]" placeholder="15" value="<?php echo esc_attr( $shipping['weight'] ); ?>" size="4" />
				</div>
			</td>
			<td<?php echo $class; ?>>
				<div class="cell-wrapper">
			...
			<td<?php echo $class; ?>>
				<div class="cell-wrapper">
					<input type="text" name="wpsc_shipping_inpostshipping_shipping[sizec]" placeholder="38x38x64" value="<?php echo esc_attr( $shipping['sizec'] ); ?>" size="10" />
				</div>
			</td>
			<td<?php echo $class; ?>>
				<div class="cell-wrapper">
					<select id="label_type" name="wpsc_shipping_inpostshipping_shipping[label_type]">
					<option value="">Please Select</option>
					<?php foreach ( array_keys( (array) $this->label_types ) as $dkey ): ?>
<?php error_log("dbkey = " . $dkey . " sel2_drop = " . $sel2_drop ); ?>
						<option value="<?php esc_attr_e( $dkey ); ?>" <?php selected( $dkey, $sel2_drop ); ?> >
							<?php echo( $this->label_types[$dkey] ); ?>
						</option>
					<?php endforeach; ?>
					</select>
				</div>
	...
	function getForm()
	{
		...
		$this->label_types = array(
			'pdf'  => __( 'Laser Printer', 'ipsp' ),
			'Epl2' => __( 'Barcode Printer', 'ipsp' ),
		);
		...
		{
			$layers['sizec'] = '38x38x64';
		}
		if(!isset($layers['label_type']))
		{
			$layers['label_type'] = 'pdf';
		}
				...
				<table>
					<thead>
						<tr>
							...
							<?php _e( 'Max Weight', 'ipsp' ); ?><br /><small><?php _e( 'in kilos (* 1000 for grams)', 'ipsp' ); ?></small>
							</th>
							...
							<th class="label-type" title="<?php _e('The default labels are PDFs for a laser printer', 'ipsp'); ?>"><?php _e( 'Label Type', 'ipsp' ); ?>
							</th>
						</tr>
					</thead>
